...
function Choices({ player, game }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-20">
      ...
      <div>{list[player]?.value}</div>
      <div>{list[housepicked]?.value}</div>
      ...
      {showResult && (
        ...
        >
          <div className="bg-[#141539] text-white w-[350px] p-4 rounded-md flex flex-col justify-center text-center text-[1.5rem] md:text-[2.5rem]  tracking-wider gap-8  ">
            <p>{resultMessage}</p>
            <p>{emoji}</p>
            <button
              onClick={() => {
